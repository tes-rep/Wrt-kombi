...
custom_packages() {
    ...
    if [[ "$ARCH_3" == "x86_64" ]]; then
        clash_meta="$(meta_api="https://api.github.com/repos/tes-rep/mihomo/releases" && meta_file="mihomo-linux-$ARCH_1-compatible-alpha-smart" && curl -s ${meta_api} | grep "browser_download_url" | grep -oE "https.*${meta_file}-[a-z0-9]+\.gz" | head -n 1)"
    else
        clash_meta="$(meta_api="https://api.github.com/repos/tes-rep/mihomo/releases" && meta_file="mihomo-linux-$ARCH_1-alpha-smart" && curl -s ${meta_api} | grep "browser_download_url" | grep -oE "https.*${meta_file}-[a-z0-9]+\.gz" | head -n 1)"
    fi
